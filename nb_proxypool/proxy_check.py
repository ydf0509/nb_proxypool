...
from nb_proxypool.proxy_pool_config import (REQUESTS_TIMEOUT, redis_client, PROXY_KEY_IN_REDIS_DEFAULT)
...
def check_one_new_proxy(proxy_dict, is_save_to_db=True,exist_proxy=False):
    ...
    if is_save_to_db and is_valid:
        redis_client.zadd(PROXY_KEY_IN_REDIS_DEFAULT, {json.dumps(proxy_dict, ensure_ascii=False): time.time()})
    if is_save_to_db and is_valid is False:
        redis_client.zrem(PROXY_KEY_IN_REDIS_DEFAULT, json.dumps(proxy_dict, ensure_ascii=False))
    return is_valid
...
def scan_exists_proxy():
    proxy_dict_str_list = redis_client.zrangebyscore(PROXY_KEY_IN_REDIS_DEFAULT, 0, time.time() - 5)
